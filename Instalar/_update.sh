...
# Description
# Baja la traducción al español latino del repositorio oficial de Rimworld y la instala en el directorio del juego ya existente.
# NOTA: El repositorio oficial contiene tan solo la traducción que se despliega con las actualizaciones release.
# NOTA2: EL SCRIPT SOLO SIRVE PARA VERSIÓN STEAM DEL JUEGO
...
function updateTranslation() {
    ...
    local translation_label="Spanish (Español(Latinoamérica))"
    ...
    # Borramos el directorio "Spanish (Español(Latinoamérica))" si ya existía
    if [ -d "${translation_path}/${translation_label}" ]; then
        rm -r "${translation_path}/${translation_label}"
    fi
    # Lo mismo con la traducción empaquetada antigua "Spanish (Español(Latinoamérica)).tar"
    if [ -f "${translation_path}/${translation_label}.tar" ]; then
        rm "${translation_path}/${translation_label}.tar"
    fi
    # Movemos las traducciones del juego base/DLCs hacia su directorio pertinente dentro del directorio de la traducción al español latino
